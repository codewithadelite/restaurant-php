...
if(isset($_SESSION['Name']) && isset($_SESSION['Id']))
{
    if(isset($_GET['reid'])){
        ...
        $del = Mysqli_query($conn,"DELETE FROM `request` WHERE `Id` = '".$id."' ");
        if($del){
            echo "<script>alert('Record confirmed');</script>";
        }
        else{
            echo "<script>alert('Record not confirmed');</script>";
        }
    }
        ...
        <nav class="navbar-default navbar-static-side" role="navigation">
            ...
            <div class="sidebar-collapse">
                ...
                <ul class="nav" id="side-menu">
                    ...
                    <li >
                        <a href="request_manage.php" class="selected"><i class="fa fa-table fa-fw"></i>Requests</a>
                    </li>
                    <li >
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                        <?php
                                            $sql = Mysqli_query($conn,"SELECT * FROM `request` ORDER BY `Id` DESC");
                                            while ($row = Mysqli_fetch_array($sql)) {
                                                echo'<tr class="odd gradeX">';
                                                echo'<td>'.$row["Names"].'</td>';
                                                echo'<td>'.$row["Table_category"].'</td>';
                                                echo'<td>'.$row["Date"].'</td>';
                                                echo'<td>'.$row["Time"].'</td>';
                                                echo'<td class="text-center">'.$row["Confirmation"].'</td>';
                                                echo'<td class="text-center">
                                                <a href="request_manage.php?reid='.$row['Id'].'"><button class="btn btn-success"><i class="fa fa-check"></i></button></td>';
                                                echo'</tr>';
                                            }
                                        ?>
                                    </tbody>
    ...
    <div class="modal fade" id="change_pass" style="width:40%; margin:auto;">
        <div clas="modal-dialog" >
            <div class="modal-content">
                ...
                <form method="POST" action="request_manage.php">
